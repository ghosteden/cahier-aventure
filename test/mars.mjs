...
await new Promise(r=>srv.listen(8234,r));
...
await page.goto("http://localhost:8234/index.html",{waitUntil:"domcontentloaded"});
...
await page.evaluate(()=>{const s=CV.Store.current(); s.currentDay=40; s.heroWorld=4; s.heroNode={4:3}; CV.Store.save(); location.reload();});
await page.waitForSelector(".map-viewport");
...
await page.screenshot({path:path.join(SHOTS,"mars-01.png")});
...
await page.screenshot({path:path.join(SHOTS,"mars-02.png")});
...
await page.screenshot({path:path.join(SHOTS,"mars-03.png")});

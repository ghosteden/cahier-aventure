...
await new Promise(r=>srv.listen(8223,r));
...
try{
  await page.goto("http://localhost:8223/index.html",{waitUntil:"domcontentloaded"});
  ...
  await page.evaluate(()=>{const s=CV.Store.current(); s.currentDay=38; CV.Store.save(); location.reload();});  // Vénus = pierre 2
  await page.waitForSelector(".map-viewport");
  ...
  console.log("Nuages posés :", await page.locator(".planet-prop").count());
  await page.screenshot({path:path.join(SHOTS,"venus-01-depart.png")});
  ...
  await page.screenshot({path:path.join(SHOTS,"venus-02-dissipe.png")});
